...
export default function EmailVerificationPage() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const verificationCode = code.join(""); // join the code array to form a string
        try {
            await verifyEmail(verificationCode); // call the verifyEmail function
            navigate('/'); // navigate to the login page
            toast.success('Email Verified Successfully'); // show a success toast message
        } catch (error) {
            console.log(error)
        }
    }
            ...
            className='bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden p-8 w-full max-w-md'>
                ...
                <form onSubmit={handleSubmit} className='space-y-6'>
                    ...
                    {code.map((digit, index) => (
                      ...
                      className='w-12 h-12 text-center text-2xl font-bold bg-gray-700 text-white border-2 border-gray-200 rounded-lg focus:border-green-500 focus:outline-none'/>      
                    ))}
                    </div>
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      type='submit'
                      disabled={isLoading || code.some((digit) => !digit)} // disable the button if the code is empty or the loading state is true
